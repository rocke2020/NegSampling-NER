...
if __name__ == "__main__":
    ...
    lexical_vocab = UnitAlphabet(os.path.join(args.resource_dir, "bert-base-cased"))
    ...
    bert_path = os.path.join(args.resource_dir, "bert-base-cased")
    ...
    optimizer = AdamW(grouped_param, lr=1e-5)
    ...
    for epoch_i in range(0, args.epoch_num + 1):
        ...
        logger.info("[Epoch {:3d}] loss on train set is {:.5f} using {:.3f} secs".format(epoch_i, loss, train_time))

        dev_f1, dev_time = Procedure.test(model, dev_loader, script_path)
        logger.info("(Epoch {:3d}) f1 score on dev set is {:.5f} using {:.3f} secs".format(epoch_i, dev_f1, dev_time))

        test_f1, test_time = Procedure.test(model, test_loader, script_path)
        logger.info("{{Epoch {:3d}}} f1 score on test set is {:.5f} using {:.3f} secs".format(epoch_i, test_f1, test_time))

        if dev_f1 > best_dev:
            best_dev = dev_f1
            checkpoint_path = os.path.join(args.check_dir, f"model_epoch_{epoch_i}_{test_f1:.5f}.pt")
